...
const RecipeDetailScreen = () => {
  ...
  useEffect(() => {
    const checkIfSaved = async () => {
      try {
        const { data } = await axios.get(`${API_URL}/recipe/favorites/${userId}`);

        const favorites = data.recipes || [];
        const isRecipeSaved = favorites.some((fav) => fav.recipeId === parseInt(recipeId));
        setIsSaved(isRecipeSaved);
      } catch (error) {
        return Alert.alert("Error", "Unable to check saved status.");
      }
    };
  ...
  const handleToggleSave = async () => {
    if (isSaving) return;               // Prevent double-taps while a request is in flight
    setIsSaving(true);

    const id = Number(recipeId);        // Ensure recipeId is numeric

    try {
      if (isSaved) {
        // --- REMOVE from favourites ------------------------------------------
        const { data } = await axios.delete(
          `${API_URL}/recipe/favorites/${recipeId}/${userId}`
        );

        if (!data?.success) throw new Error("Unable to remove recipe from favourites");

        setIsSaved(false);
        Alert.alert("Success", "Recipe removed from favourites");
      } else {
        // --- ADD to favourites -----------------------------------------------
        const { data } = await axios.post(
          `${API_URL}/recipe/favorites`,
          {
            userId,
            recipeId: id,
            title: recipe.title,
        ...
        if (!data?.success) throw new Error("Unable to save recipe");

        setIsSaved(true);
        Alert.alert("Success", "Recipe added to favourites");
      }
    } catch (err) {
      const msg =
